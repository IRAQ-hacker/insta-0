#! /usr/bin/env python3
# InstagramOSINTram Scraper
# Coded by ReKuShE 
# https://github.com/iraq-hacker/insta-0
...
class InstagramOSINT:
    ...
    def __init__(self, username, downloadPhotos):
        ...
        r = requests.get(f'http://InstagramOSINTram.com/{self.username}', headers={'User-Agent': random.choice(self.useragents)})
        ...
        if self.profile_data['is_private'].lower() == 'true':
            ...
            for index, post in enumerate(self.profile_meta['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges']):
                ...
                with open(f'{os.getcwd()}/{index}/' + ''.join([random.choice(string.ascii_uppercase) for x in range(random.randint(1, 9))]) + '.jpg', 'wb') as f:
                    # Delay the request times randomly (be nice to InstagramOSINTram)
    ...
    def print_data(self):
        ...
        print(colors.OKGREEN + f"Results: scan for {self.profile_data['Username']} on InstagramOSINTram" + colors.ENDC)
...
def parse_args():
    parser = argparse.ArgumentParser(description="InstagramOSINTram OSINT tool")
